...
To ensure the code is easily extensible, the following patterns were implemented:
...
- **Net Price vs. Total Price:** Since providers send different price keys (e.g., net_price vs. total), the system prioritizes the "Total Price" including taxes to ensure transparency for the user.
- **Deduplication Logic:** A room is considered a duplicate if both the "Hotel Name" and "Room Code" match exactly across different providers.
- **Reliability:** If a specific API is down, the system logs the error and proceeds with data from the remaining functional APIs instead of failing the entire request.
...
**Testing**
    ...
    The project includes Feature Tests to ensure API integrity and business logic correctness.
